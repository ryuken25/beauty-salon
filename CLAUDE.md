...
- Baseline: `2026-05-12-100000_ResetAndCreateSalonSchema.php`. Latest: `2026-09-14-100000_DropRedundantBookingPriceColumns.php` (buang `final_service_price`, `promo_id`, `promo_discount_type` dari `bookings` — ketiganya duplikat `harga_layanan`/`layanan_id`/konstanta; `bookings` jadi 34 kolom). Snapshot harga yang DIPERTAHANKAN: `original_service_price`, `promo_name`, `promo_discount_value`, `remaining_payment`. Add NEW dated migrations for any further schema change — **never** rely on `$db->getFieldNames()` inside a migration (CI4 caches it for the whole migrate run; probe `information_schema` instead).
