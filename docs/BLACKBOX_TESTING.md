...
| ID | Skenario | Input | Output diharapkan | Hasil |
|---|---|---|---|---|
| BB-30 | Login berhasil (pemilik) | Email `owner@…`, pwd `Password123!` | Redirect `/admin/dashboard`, sidebar lengkap (Layanan, Transaksi, Pengaturan) | |
...
| BB-48 | Jadwal harian | GET `/admin/booking/jadwal?tanggal=2026-05-15` | Tabel timeline per slot 30 menit dengan kode booking + nama + layanan + status | |
...
| BB-84 | Admin dashboard limited | Login sebagai admin | Tidak tampil card Pendapatan, tidak ada chart pendapatan (akses pemilik only) | |

## E. Penyelesaian booking & pencatatan transaksi (modal)

| ID | Skenario | Input | Output diharapkan | Hasil |
|---|---|---|---|---|
| BB-100 | Buka modal | Booking `accepted`, klik tombol "Selesaikan + transaksi" di /admin/booking/{id} | Modal Bootstrap muncul, base price ter-display otomatis dari `harga_layanan`, additional = 0, total = base, note tersembunyi | |
| BB-101 | Additional > 0 → note muncul | Isi `additional_price = 50000` | Field note ter-show, atribut `required`, total live = base + 50.000 | |
...
| BB-91 | Seeder clean | `php spark db:seed SalonSeeder` setelah migrate fresh | Demo data terisi, 2 user + 24 layanan + settings | |
