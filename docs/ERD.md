...
- **`bookings`** — booking utama. `user_id` nullable (booking anonim tetap diizinkan). Otentikasi customer anonim via kombinasi `(kode_booking, nomor_hp_pelanggan)`. **Tidak ada `stylist_id` — owner all-in-one, jam buka/tutup diambil dari `settings`.**
...
booking_slots(id PK, booking_id FK, tanggal, slot_waktu, status ENUM[held,released], created_at)
